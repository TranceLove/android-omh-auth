...
internal class AuthRepositoryTest {
    ...
    fun `GIVEN all parameters, WHEN response successful, THEN tokens stored and returned`() =
        runTest {
            ...
            coEvery {
                microsoftAuthDataSource.requestToken(
                    clientId = any(),
                    authCode = any(),
                    redirectUri = any(),
                    codeVerifier = any(),
            ...
            coVerify {
                microsoftAuthDataSource.storeToken(
    ...
    fun `GIVEN all parameters, WHEN build URL requested, THEN login URL returned`() = runTest {
        ...
        every { microsoftAuthDataSource.buildLoginUrl(any(), any(), any(), any()) } returns uri
    ...
    fun `GIVEN all parameters, WHEN access token requested, THEN access token returned`() =
        runTest {
            ...
            every { microsoftAuthDataSource.getToken(any()) } returns accessToken
    ...
    fun `GIVEN client ID, WHEN token refresh requested, THEN access token returned and stored`() =
        runTest {
            ...
            coEvery { microsoftAuthDataSource.refreshAccessToken(any()) } returns responseResult

            val result: ApiResult<String> = authRepository.refreshAccessToken(clientId)

            coEvery { microsoftAuthDataSource.storeToken(AuthDataSource.ACCESS_TOKEN, accessToken) }
    ...
    fun `GIVEN access token WHEN token revoke requested, THEN token revoked`() = runTest {
        ...
        every { microsoftAuthDataSource.getToken(any()) } returns accessToken
        coEvery { microsoftAuthDataSource.revokeToken(any()) } returns ApiResult.Success(Unit)
    ...
    fun `GIVEN no access token WHEN token revoke requested, THEN exception thrown`() = runTest {
        ...
        every { microsoftAuthDataSource.getToken(any()) } returns null
    ...
    fun `WHEN clear data requested THEN data cleared`() = runTest {
        ...
        every { microsoftAuthDataSource.clearData() } returns Unit

        authRepository.clearData()

        verify { microsoftAuthDataSource.clearData() }
    }
}
